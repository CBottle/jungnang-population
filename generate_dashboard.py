...
body{font-family:'Malgun Gothic','Apple SD Gothic Neo',sans-serif;background:#f0f2f5;color:#333;height:100vh;display:flex;flex-direction:column;overflow:hidden}

header{background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;padding:12px 20px;display:flex;align-items:center;gap:12px;flex-shrink:0}
header h1{font-size:1.2rem;font-weight:700}
header p{font-size:0.78rem;opacity:.65;margin-top:2px}

.controls{background:#fff;padding:10px 20px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;border-bottom:1px solid #e0e0e0;box-shadow:0 1px 4px rgba(0,0,0,.06);flex-shrink:0}
.cg{display:flex;align-items:center;gap:6px}
.cg label{font-size:.78rem;font-weight:700;color:#555;white-space:nowrap}
select{padding:5px 8px;border:1px solid #ddd;border-radius:6px;font-size:.8rem;background:#fafafa;cursor:pointer;outline:none;font-family:inherit}
select:focus{border-color:#4a90e2}
.mode-toggle{display:flex;border:1px solid #ddd;border-radius:6px;overflow:hidden}
.mbtn{padding:5px 12px;font-size:.78rem;cursor:pointer;border:none;background:#fafafa;color:#555;font-family:inherit;transition:all .15s}
.mbtn.active{background:#4a90e2;color:#fff;font-weight:700}

.main{display:flex;flex:1;min-height:0;padding:10px;gap:10px}
.map-wrap{flex:1;min-width:0;border-radius:10px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.1);background:#fff}
#map{width:100%;height:100%}

.sidebar{width:360px;display:flex;flex-direction:column;gap:8px;overflow:hidden}
.card{background:#fff;border-radius:10px;padding:12px 14px;box-shadow:0 1px 6px rgba(0,0,0,.08);display:flex;flex-direction:column}
.card-title{font-size:.75rem;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px}
.chart-wrap{flex:1;min-height:0}
#card-stats{flex-shrink:0}
#card-bar{flex:1.1}
#card-line{flex:1}

.hint{font-size:.78rem;color:#bbb;text-align:center;padding:16px 0}
.stat-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:4px}
.stat-item{background:#f8f9fa;border-radius:6px;padding:7px 9px}
.stat-item .sl{font-size:.68rem;color:#999;margin-bottom:1px}
.stat-item .sv{font-size:.95rem;font-weight:700}
.pos{color:#e74c3c}.neg{color:#3498db}

.year-bar{background:#fff;border-top:1px solid #e0e0e0;padding:8px 20px 10px;display:flex;align-items:center;gap:12px;flex-shrink:0}
.yl{font-size:.78rem;font-weight:700;color:#555;white-space:nowrap}
.yv{font-size:1rem;font-weight:700;color:#4a90e2;min-width:44px;text-align:center}
input[type=range]{flex:1;-webkit-appearance:none;height:4px;background:#ddd;border-radius:2px;outline:none;cursor:pointer}
input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:#4a90e2;cursor:pointer;box-shadow:0 1px 4px rgba(0,0,0,.3)}
.play-btn{width:28px;height:28px;border:none;background:#4a90e2;color:#fff;border-radius:50%;cursor:pointer;font-size:11px;flex-shrink:0;display:flex;align-items:center;justify-content:center}
.year-ticks{font-size:.7rem;color:#aaa;display:flex;justify-content:space-between;width:100%;position:relative}
</style>
</head>
<body>
...
<div class="controls">
  ...
  <div class="cg" style="margin-left:auto;font-size:.75rem;color:#aaa">
    * 지도를 클릭하면 동별 상세 분석
  </div>
</div>
...
function updateBar(dong, y) {
  ...
  Plotly.react('barChart',[
    {type:'bar',name:'남성',y:ags,x:maleV,orientation:'h',marker:{color:'#4a90e2',opacity:.85}},
    {type:'bar',name:'여성',y:ags,x:femV,orientation:'h',marker:{color:'#e88a9a',opacity:.85}},
  ],{
    barmode:'overlay',
    margin:{t:4,b:28,l:60,r:4},
    xaxis:{
      tickfont:{size:8},
      tickformat:',d',
      title:{text:'← 남성 &nbsp;&nbsp; 여성 →',font:{size:9}},
...
function updateLine(dong) {
  ...
  Plotly.react('lineChart',traces,{
    ...
    xaxis:{tickvals:YEARS,ticktext:YEARS.map(String),tickfont:{size:8}},
    yaxis:{tickfont:{size:8},title:{text:'인구수',font:{size:9}}},
    legend:{font:{size:8},orientation:'h',y:-0.35,tracegroupgap:0},
    paper_bgcolor:'transparent',plot_bgcolor:'transparent',
    height:undefined,
  },{responsive:true,displayModeBar:false});
}
